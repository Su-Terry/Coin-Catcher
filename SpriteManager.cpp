...
void SpriteManager::SpawnBomb()
{
	...
	sprites[3].push_back(new Bomb(bomb));
}
...
void SpriteManager::DrawFinalScoreText()
{
	int score = CountScore();

	char scoreString[20];
	sprintf(scoreString, "Score: %d", score);
	gltSetText(finalScoreText, scoreString);

	gltBeginDraw();

	gltColor(1.0f, 0.0f, 0.0f, 1.0f);
	gltDrawText2DAligned(finalScoreText, 500.0f, 400.0f, 4.0f, GLT_CENTER, GLT_CENTER);
...
void SpriteManager::MV()
{
	if (IsAllDead()) {
		for (int i = 0; i < sprites[2].size(); ++i) {
			sprites[2][i]->MV();
		}
	}
	else {
